...
huffman *huffman_node_init(char *word,int number);
...
int main(){
    ...
    int size = (sizeof(data*) + sizeof(huffman*)) * 1000 + 1;
    printf("size:%d\n",size);//普通数组创建方法使用的总长度是4000，我的天malloc你个大骗子

    data *bucket[500];//(data*)malloc(size);
...
int huffman_create(data *bucket,char *word[],int rate[],int len){
    ...
    for(;i < len - 1;i++){
        ...
        if(bucket[rate[min]].pointer != NULL && bucket[rate[min]].pointer->num == rate[min]){//如果最小值是前父树（新二叉树左侧）
            //左侧是老父树，右侧空
            bucket[rate[min2]].pointer = huffman_node_init(word[min2],rate[min2]);//right
            //他俩的父节点的num属性就是他俩的频率和
            int parent_num = rate[min]+rate[min2];
            bucket[parent_num].pointer = huffman_node_init(NULL,parent_num);
            //左右节点分配
            ...
            //右侧是老父树，左侧空
            bucket[rate[min]].pointer = huffman_node_init(word[min],rate[min]);//left
            //他俩的父节点的num属性就是他俩的频率和
            int parent_num = rate[min]+rate[min2];
            bucket[parent_num].pointer = huffman_node_init(NULL,parent_num);
            //左右节点分配
